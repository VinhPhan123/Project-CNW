...
<?php 
	...
	// tạo 1 array lưu id_major - major_name tương ứng trong bảng majors
                        ...
                        <?php
                            foreach($teachers as $teacher => $email){
                                    echo  '<tr id="id_' . $j . '">';
										...
										$query = select('teachers', '*', ['username'=>$teacher]);
										$id_teacher = mysqli_fetch_array($query)['id_teacher'];
										$arr_id_major_by_id_teacher = array();
										$arr_id_major_by_id_teacher = isset($idTeacher_listIdMajor_array[$id_teacher]) ? $idTeacher_listIdMajor_array[$id_teacher] : array();
										$arr_major_name_of_teacher = array();
										foreach($arr_id_major_by_id_teacher as $major_id){
											array_push($arr_major_name_of_teacher, $idMajor_major_array[$major_id]);
										}										
                                        ...
                                        echo '<td>
											...
											<form class="form_delete" method="post">
												...
												<select name="tohop_delete" class="select_tohop">
													<option></option>';?>
													<?php foreach($arr_major_name_of_teacher as $major) { ?>
														<?php echo '<option value="' . $major .'">' . $major . '</option>'?>
													<?php } ?>
												<?php
												echo '</select>';
